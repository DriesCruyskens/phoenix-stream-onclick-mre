/*
 *   Slight modification: changed click events from "click contextmenu" to "mousedown"
 *   The "click" event would not fire when clicking on a marker. Ideally the visual click
 *   would only show when clicking on a marker but I couldn't find a way so on every
 *   "mousedown" event is good enough for now.
 *
 *   Also changed the pane from shadowPane to popupPane so it is more visible over the markers.
 *
 * L.VisualClick
...
L.Map.VisualClick = L.Handler.extend({
  _makeVisualIcon: function () {
    var touchMode =
      this._map.options.visualClickMode === "touch" ? true : false;
  ...
  _onClick: function (e) {
    ...
    window.setTimeout(
      ...
      map.options.visualClick.removeTimeout || 450
    ); // Should somewhat match the css animation to prevent loops
...
L.Map.mergeOptions({
  ...
  visualClickEvents: "mousedown", //Standard leaflety way of defining which events to hook on to
  visualClickPane:
    L.Browser.ie && document.documentMode === 10
      ? "ie10-visual-click-pane"
      : "popupPane", // Map pane where the pulse markers will be shown
});
